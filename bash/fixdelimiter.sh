...
for p in $patients #loop over 10 participants.
do
	for a in $stages #loop over wr and NREMe
	do
		for lam in {2..9} #loop over 8 history lengths	
		do
			for ch in {0..30} #loop over 31 channels
			do
				filename="../cssr_output/${p}/${p}_${a}_channel_${ch}_2500_${lam}" # "$filename"
				echo 'attempting delimiter for '"$filename"' ok here we go'
				###

				#Check that the *_state_series file exists so can fix delimiter. If it does not exist we skip to the next .
				# else skip
